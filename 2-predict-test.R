...
missing_cols <- setdiff(names(cat_mm), names(test_cat_mm1))
...
# Fit m1 =======================================================================
df_test1 <- data.frame(cbind(test_cat_mm1, num_test1))
...
test_out <- data.frame(rbind(test_out1, test_out2))
colnames(test_out) <- c('ID', 'y')
test_out$y <- exp(test_out$y)

write.csv(test_out, 'output/log-10.csv', row.names = FALSE)
